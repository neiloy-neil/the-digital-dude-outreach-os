...
export async function POST(request: Request) {
  ...
  try {
    const { leadId, campaignId, requestedDepth, requestedMode } = await request.json();
    ...
    const dailyCalls = dailyUsage.count || 0;
    const monthlyCalls = monthlyUsage.count || 0;
    ...
    const deepLimitExceeded =
      ...
      dailyDeepLimit > 0 &&
      (dailyDeepUsage.count || 0) >= dailyDeepLimit;
    ...
    if (deepLimitExceeded) {
      usageSkipReason = `Deep AI daily limit reached (${dailyDeepUsage.count || 0}/${dailyDeepLimit}).`;
    }
